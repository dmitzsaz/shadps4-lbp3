...
EnumeratePhysicalDevices(vk::UniqueInstance &instance) {
  auto [devices_result, devices] = instance->enumeratePhysicalDevices();
  ASSERT_MSG(devices_result == vk::Result::eSuccess,
...
std::vector<std::string> GetSupportedExtensions(vk::PhysicalDevice physical) {
  const auto [extensions_result, extensions] =
...
vk::FormatProperties3 GetFormatProperties(vk::PhysicalDevice physical,
                                          vk::Format format) {
...
std::unordered_map<vk::Format, vk::FormatProperties3>
GetFormatProperties(vk::PhysicalDevice physical) {
  ...
  // Other miscellaneous formats, e.g. for color buffers, swizzles, or
  // compatibility
...
std::string GetReadableVersion(u32 version) {
  return fmt::format("{}.{}.{}", VK_VERSION_MAJOR(version),
...
Instance::Instance(bool enable_validation, bool enable_crash_diagnostic)
    : instance{CreateInstance(Frontend::WindowSystemType::Headless,
                              enable_validation, enable_crash_diagnostic)},
...
Instance::Instance(Frontend::WindowSDL &window, s32 physical_device_index,
  ...
  const std::size_t num_physical_devices =
      static_cast<u16>(physical_devices.size());
  ...
  if (physical_device_index < 0) {
    std::vector<std::tuple<size_t, vk::PhysicalDeviceProperties2,
    ...
    std::sort(
        properties2.begin(), properties2.end(),
        [](const auto &left, const auto &right) {
          const vk::PhysicalDeviceProperties &left_prop =
              std::get<1>(left).properties;
          const vk::PhysicalDeviceProperties &right_prop =
              std::get<1>(right).properties;
          const bool left_supports_api =
              left_prop.apiVersion >= TargetVulkanApiVersion;
          const bool right_supports_api =
          ...
          constexpr auto get_mem =
              [](const vk::PhysicalDeviceMemoryProperties &mem) -> size_t {
            size_t max = 0;
...
bool Instance::SupportsExternalMemoryHost() const noexcept {
  return std::find(available_extensions.begin(), available_extensions.end(),
                   VK_EXT_EXTERNAL_MEMORY_HOST_EXTENSION_NAME) !=
         available_extensions.end();
}

vk::DeviceSize Instance::MinImportedHostPointerAlignment() const noexcept {
  ...
  return host_properties.minImportedHostPointerAlignment;
}
...
bool Instance::CreateDevice() {
  ...
  push_descriptor_props =
      properties_chain.get<vk::PhysicalDevicePushDescriptorPropertiesKHR>();
  LOG_INFO(Render_Vulkan, "Physical device subgroup size {}",
  ...
  const auto robustness2_features =
      feature_chain.get<vk::PhysicalDeviceRobustness2FeaturesEXT>();
  ...
  attachment_feedback_loop =
      add_extension(VK_EXT_ATTACHMENT_FEEDBACK_LOOP_LAYOUT_EXTENSION_NAME);
  if (attachment_feedback_loop) {
    attachment_feedback_loop = add_extension(
        VK_EXT_ATTACHMENT_FEEDBACK_LOOP_DYNAMIC_STATE_EXTENSION_NAME);
    if (!attachment_feedback_loop) {
      // We want both extensions so remove the first if the second isn't
      // available
  ...
  if (list_restart) {
    list_restart_features =
        feature_chain
            .get<vk::PhysicalDevicePrimitiveTopologyListRestartFeaturesEXT>();
  ...
  shader_stencil_export =
      add_extension(VK_EXT_SHADER_STENCIL_EXPORT_EXTENSION_NAME);
  image_load_store_lod =
      add_extension(VK_AMD_SHADER_IMAGE_LOAD_STORE_LOD_EXTENSION_NAME);
  amd_gcn_shader = add_extension(VK_AMD_GCN_SHADER_EXTENSION_NAME);
  amd_shader_trinary_minmax =
      add_extension(VK_AMD_SHADER_TRINARY_MINMAX_EXTENSION_NAME);
  ...
  shader_atomic_float =
      add_extension(VK_EXT_SHADER_ATOMIC_FLOAT_EXTENSION_NAME);
  shader_atomic_float2 =
      add_extension(VK_EXT_SHADER_ATOMIC_FLOAT_2_EXTENSION_NAME);
  ...
  if (workgroup_memory_explicit_layout) {
    workgroup_memory_explicit_layout_features =
        feature_chain
            .get<vk::PhysicalDeviceWorkgroupMemoryExplicitLayoutFeaturesKHR>();
  ...
  image_2d_view_of_3d =
      add_extension(VK_EXT_IMAGE_2D_VIEW_OF_3D_EXTENSION_NAME);
  ...
  vk::StructureChain device_chain = {
      ...
      vk::PhysicalDeviceVulkan12Features{
          ...
          .uniformBufferStandardLayout =
              vk12_features.uniformBufferStandardLayout,
  ...
  if (!list_restart) {
    device_chain
        .unlink<vk::PhysicalDevicePrimitiveTopologyListRestartFeaturesEXT>();
  }
  if (!fragment_shader_barycentric) {
    device_chain
        .unlink<vk::PhysicalDeviceFragmentShaderBarycentricFeaturesKHR>();
  }
  if (!provoking_vertex) {
  ...
  if (!attachment_feedback_loop) {
    device_chain
        .unlink<vk::PhysicalDeviceAttachmentFeedbackLoopLayoutFeaturesEXT>();
    device_chain.unlink<
        vk::PhysicalDeviceAttachmentFeedbackLoopDynamicStateFeaturesEXT>();
  }
  if (!shader_atomic_float2) {
  ...
  if (!workgroup_memory_explicit_layout) {
    device_chain
        .unlink<vk::PhysicalDeviceWorkgroupMemoryExplicitLayoutFeaturesKHR>();
  }
  if (!image_2d_view_of_3d) {
  ...
  auto [device_result, dev] =
      physical_device.createDeviceUnique(device_chain.get());
  ...
  if (calibrated_timestamps) {
    ...
    if (time_domains_result == vk::Result::eSuccess) {
      ...
      if (has_host_time_domain) {
        ...
        TracyVkContextName(profiler_context, context_name.data(),
                           context_name.size());
      }
    } else {
      LOG_WARNING(Render_Vulkan,
                  "Could not query calibrated time domains for profiling: {}",
...
void Instance::CollectDeviceParameters() {
  ...
  const std::string driver_name =
      fmt::format("{} {}", vendor_name, driver_version);
...
void Instance::CollectPhysicalMemoryInfo() {
  ...
  for (size_t i = 0; i < num_props; ++i) {
    ...
    // If memory budget is not supported, use the size of the heap as the
    // budget.
  ...
  const s64 available_memory =
      static_cast<s64>(total_memory_budget - device_initial_usage);
...
void Instance::CollectImageFormatInfo() {
  ...
  for (const auto &format : LiverpoolToVK::SurfaceFormats()) {
    if (!IsFormatSupported(format.vk_format, format.flags)) {
      LOG_WARNING(Render_Vulkan,
                  "Surface format data_format={}, number_format={} is not "
                  "fully supported "
                  "(vk_format={}, missing features={})",
                  static_cast<u32>(format.data_format),
                  static_cast<u32>(format.number_format),
                  vk::to_string(format.vk_format),
                  vk::to_string(format.flags &
                                ~GetFormatFeatureFlags(format.vk_format)));
    }
  }
  for (const auto &format : LiverpoolToVK::DepthFormats()) {
    if (!IsFormatSupported(format.vk_format, format.flags)) {
      LOG_WARNING(
          ...
          "(vk_format={}, missing features={})",
          static_cast<u32>(format.z_format),
          static_cast<u32>(format.stencil_format),
          vk::to_string(format.vk_format),
          vk::to_string(format.flags &
                        ~GetFormatFeatureFlags(format.vk_format)));
    }
...
Instance::GetFormatFeatureFlags(vk::Format format) const {
  const auto it = format_properties.find(format);
  if (it == format_properties.end()) {
    UNIMPLEMENTED_MSG("Properties of format {} have not been queried.",
                      vk::to_string(format));
  }
...
vk::Format
Instance::GetSupportedFormat(const vk::Format format,
                             const vk::FormatFeatureFlags2 flags) const {
