...
static constexpr size_t FaultStormDownloadBufferSize = 64_MB;
...
BufferCache::BufferCache(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_,
    ...
    : instance{instance_}, scheduler{scheduler_}, liverpool{liverpool_},
      ...
      fault_storm_download_buffer{instance, scheduler, MemoryUsage::Download,
                                  FaultStormDownloadBufferSize},
...
void BufferCache::InvalidateMemory(VAddr device_addr, u64 size, bool download) {
    ...
    if (download) {
        ...
        CancelPreemptiveDownloads(device_addr, size);
...
void BufferCache::ReadMemory(VAddr device_addr, u64 size, bool is_write) {
    if (use_lbp3_preemptive_readbacks && is_write) {
        QueueFaultReadback(device_addr, size);
        return;
    }
    ...
    liverpool->SendCommand<true>([this, device_addr, size, is_write] {
        ...
        if (buffer.IsHostImported()) {
            if (!is_write) {
                DownloadBufferMemory<false>(buffer, device_addr, size, false);
                return;
            }
        ...
        if (is_write && !IsKosmicKrisp(instance)) {
            ...
            if (!state.attempted && ++state.write_faults >= HostImportWriteFaultThresholdV159) {
                ...
                if (instance.SupportsExternalMemoryHost() && fully_mapped) {
                    ...
                    DownloadBufferMemory<false>(buffer, buffer.CpuAddr(), buffer.SizeBytes(), true);
                    scheduler.Finish();
...
void BufferCache::QueueFaultReadback(VAddr device_addr, u64 size) {
    FaultReadbackRequest request{
        .device_addr = device_addr,
        .size = size,
    };
    bool schedule_batch = false;
    {
        std::scoped_lock lock{fault_readback_mutex};
        fault_readback_requests.push_back(&request);
        if (!fault_readback_scheduled) {
            fault_readback_scheduled = true;
            schedule_batch = true;
        }
    }

    if (schedule_batch) {
        liverpool->SendCommand([this] { ProcessFaultReadbackRequests(); });
    }
    request.finished.acquire();
}

void BufferCache::ProcessFaultReadbackRequests() {
    ...
    for (;;) {
        std::vector<FaultReadbackRequest*> requests;
        {
            std::scoped_lock lock{fault_readback_mutex};
            if (fault_readback_requests.empty()) {
                fault_readback_scheduled = false;
                return;
            }
            requests.swap(fault_readback_requests);
        }

        const auto now = std::chrono::steady_clock::now();
        if (last_fault_readback.time_since_epoch().count() == 0 ||
            now - last_fault_readback > FaultBurstGap) {
            fault_readback_burst_count = 0;
            fault_readback_storm = false;
            fault_readback_storm_logged = false;
        }
        last_fault_readback = now;
        fault_readback_burst_count += static_cast<u32>(requests.size());
        fault_readback_storm |= fault_readback_burst_count >= FaultStormThreshold;
        if (fault_readback_storm && !fault_readback_storm_logged) {
            fault_readback_storm_logged = true;
            LOG_INFO(Render_Vulkan,
                     "LBP3 A22 detected a sequential write-fault storm; draining up to {:#x} "
                     "bytes of global GPU-dirty buffer state with one fence",
                     FaultStormDrainBudget);
        }

        std::vector<PlannedRange> ranges;
        ranges.reserve(requests.size() + 32);
        for (FaultReadbackRequest* request : requests) {
            const BufferId buffer_id = FindBuffer(request->device_addr, request->size);
            Buffer& buffer = slot_buffers[buffer_id];
            request->buffer_addr = buffer.CpuAddr();
            request->buffer_size = buffer.SizeBytes();

            if (buffer.IsHostImported()) {
                gpu_modified_ranges.Subtract(request->device_addr, request->size);
                buffer.MarkHostWrite();
                continue;
            }
        ...
        if (fault_readback_storm && !requests.empty()) {
            // The guest faults are sequential: waiting for one page before looking for the next
            // means the request queue itself can never contain the whole burst. Snapshot all
            // currently GPU-dirty registered buffer pages and drain the closest ones now. This
            // supersedes A21's per-buffer 16 MiB widening, which still consumed one preemptive
            // fence per buffer and left 18-23 serialized GPU waits in the destruction cloud.
            std::vector<PlannedRange> dirty_ranges;
            gpu_modified_ranges.ForEach([&](VAddr dirty_begin, VAddr dirty_end) {
                for (VAddr page_begin = PageManager::GetPageAddr(dirty_begin);
                     page_begin < dirty_end; page_begin += TRACKER_BYTES_PER_PAGE) {
                    const BufferId buffer_id = page_table[page_begin >> CACHING_PAGEBITS].buffer_id;
                    ...
                    const VAddr buffer_begin = buffer.CpuAddr();
                    const VAddr buffer_end = buffer_begin + buffer.SizeBytes();
                    const VAddr begin = std::max(page_begin, buffer_begin);
                    const VAddr end = std::min<VAddr>(
                        {page_begin + TRACKER_BYTES_PER_PAGE, buffer_end,
                         Common::AlignUp(dirty_end, TRACKER_BYTES_PER_PAGE)});
                    if (begin >= end) {
                        continue;
                    }
                    if (!dirty_ranges.empty()) {
                        PlannedRange& previous = dirty_ranges.back();
                        if (previous.buffer_id == buffer_id && previous.end == begin) {
                            previous.end = end;
                            continue;
                        }
                    }
                    dirty_ranges.push_back(PlannedRange{
                        .buffer_id = buffer_id,
                        .buffer_addr = buffer_begin,
                        .begin = begin,
                        .end = end,
                        .global_drain = true,
                    });
            ...
            const VAddr trigger_addr = requests.front()->device_addr;
            const auto distance_to_trigger = [trigger_addr](const PlannedRange& range) -> u64 {
                if (trigger_addr < range.begin) {
                    return range.begin - trigger_addr;
                }
                if (trigger_addr >= range.end) {
                    return trigger_addr - range.end;
                }
                return 0;
            };
            std::ranges::sort(dirty_ranges, [&](const PlannedRange& lhs, const PlannedRange& rhs) {
                return std::tuple{distance_to_trigger(lhs), lhs.buffer_addr, lhs.begin} <
                       std::tuple{distance_to_trigger(rhs), rhs.buffer_addr, rhs.begin};
            });

            ranges.clear();
            u64 remaining_budget = FaultStormDrainBudget;
            for (PlannedRange range : dirty_ranges) {
                if (remaining_budget == 0) {
                    break;
                }
                const u64 range_size = range.end - range.begin;
                const u64 take_size = std::min(range_size, remaining_budget);
                if (take_size < range_size) {
                    // Keep the bounded slice closest to the fault that triggered this drain.
                    if (trigger_addr >= range.end) {
                        range.begin = range.end - take_size;
                    } else if (trigger_addr > range.begin) {
                        const u64 left = std::min<u64>(trigger_addr - range.begin, take_size / 2);
                        range.begin = trigger_addr - left;
                        range.begin =
                            std::min<VAddr>(range.begin, range.end - static_cast<VAddr>(take_size));
                        range.end = range.begin + take_size;
                    } else {
                        range.end = range.begin + take_size;
                    }
                }
                ranges.push_back(range);
                remaining_budget -= range.end - range.begin;
            }

            // A tracker/range-set mismatch must not strand a thread that took the original
            // fault. Keep every triggering range even if the optional global budget was filled.
            for (FaultReadbackRequest* request : requests) {
                const BufferId buffer_id = FindBuffer(request->device_addr, request->size);
                const Buffer& buffer = slot_buffers[buffer_id];
                if (buffer.IsHostImported()) {
                ...
                const bool covered = std::ranges::any_of(ranges, [&](const PlannedRange& range) {
                    return range.buffer_id == buffer_id && range.begin <= request->device_addr &&
        ...
        // Faults on adjacent pages of the same cache buffer are one readback range.
        // This removes both the per-thread Liverpool command and the per-page copy setup.
        std::ranges::sort(ranges, [](const PlannedRange& lhs, const PlannedRange& rhs) {
            return std::tie(lhs.buffer_addr, lhs.begin) < std::tie(rhs.buffer_addr, rhs.begin);
        });
        std::vector<PlannedRange> merged_ranges;
        merged_ranges.reserve(ranges.size());
        for (const PlannedRange& range : ranges) {
            if (!merged_ranges.empty()) {
                PlannedRange& previous = merged_ranges.back();
                if (previous.buffer_id == range.buffer_id && range.begin <= previous.end) {
                    previous.end = std::max(previous.end, range.end);
                    previous.global_drain |= range.global_drain;
                    continue;
                }
            }
        ...
        bool download_work_submitted = false;
        bool storm_batch_complete = false;
        if (fault_readback_storm && !merged_ranges.empty()) {
            struct StormCopy {
                BufferId buffer_id{};
                VAddr device_addr{};
                u64 size{};
            ...
            std::vector<StormCopy> storm_copies;
            std::vector<std::pair<VAddr, u64>> tracker_ranges;
            u64 total_size_bytes = 0;
            bool batch_overflow = false;
            for (const PlannedRange& range : merged_ranges) {
                memory_tracker->ForEachDownloadRange<false>(
                    range.begin, range.end - range.begin,
                    [&](VAddr tracker_addr, u64 tracker_size) {
                        tracker_ranges.emplace_back(tracker_addr, tracker_size);
                        gpu_modified_ranges.ForEachInRange(
                            tracker_addr, tracker_size, [&](VAddr copy_begin, VAddr copy_end) {
                                if (batch_overflow) {
                                    return;
                                }
                                const u64 copy_size = copy_end - copy_begin;
                                const u64 staging_offset = Common::AlignUp(total_size_bytes, 64ULL);
                                if (staging_offset + copy_size > FaultStormDownloadBufferSize) {
                                    batch_overflow = true;
                                    return;
                                }
                                storm_copies.push_back(StormCopy{
                                    .buffer_id = range.buffer_id,
                                    .device_addr = copy_begin,
                                    .size = copy_size,
                                    .staging_offset = staging_offset,
                                });
                                total_size_bytes = staging_offset + copy_size;
                            });
                    });
            }

            if (!batch_overflow && storm_copies.empty()) {
                storm_batch_complete = true;
            } else if (!batch_overflow) {
                const auto [staging, download_offset] =
                    fault_storm_download_buffer.Map(total_size_bytes, 64);
                if (staging != nullptr) {
                    fault_storm_download_buffer.Commit();
                    for (const auto [tracker_addr, tracker_size] : tracker_ranges) {
                        gpu_modified_ranges.Subtract(tracker_addr, tracker_size);
                    }
                    for (const PlannedRange& range : merged_ranges) {
                        CancelPreemptiveDownloads(range.begin, range.end - range.begin);
                    }

                    std::unordered_map<BufferId, std::vector<vk::BufferCopy>> copies_by_buffer;
                    for (const StormCopy& copy : storm_copies) {
                        Buffer& buffer = slot_buffers[copy.buffer_id];
                        auto& copies = copies_by_buffer[copy.buffer_id];
                        const vk::BufferCopy vk_copy{
                            .srcOffset = buffer.Offset(copy.device_addr),
                            .dstOffset = download_offset + copy.staging_offset,
                            .size = copy.size,
                        };
                        if (!copies.empty() &&
                            copies.back().srcOffset + copies.back().size == vk_copy.srcOffset &&
                            copies.back().dstOffset + copies.back().size == vk_copy.dstOffset) {
                            copies.back().size += vk_copy.size;
                        } else {
                            copies.push_back(vk_copy);
                        }
                    }

                    scheduler.EndRendering();
                    const auto cmdbuf = scheduler.CommandBuffer();
                    boost::container::small_vector<vk::BufferMemoryBarrier2, 16> pre_barriers;
                    for (const auto& [buffer_id, copies] : copies_by_buffer) {
                        Buffer& buffer = slot_buffers[buffer_id];
                    ...
                    if (const auto barrier = fault_storm_download_buffer.GetBarrier(
                    ...
                    for (const auto& [buffer_id, copies] : copies_by_buffer) {
                        cmdbuf.copyBuffer(slot_buffers[buffer_id].Handle(),
                                         fault_storm_download_buffer.Handle(), copies);
                    }
                    if (const auto barrier = fault_storm_download_buffer.GetBarrier(
                    ...
                    scheduler.DeferOperation(
                        [this, staging, storm_copies = std::move(storm_copies)] {
                            for (const StormCopy& copy : storm_copies) {
                                if (!memory->TryWriteBacking(
                                        std::bit_cast<u8*>(copy.device_addr),
                                        staging + copy.staging_offset, copy.size)) {
                                    LOG_ERROR(Render_Vulkan,
                                              "LBP3 A23 could not write a fault-storm readback "
                                              "to guest address {:#x} (size {:#x})",
                                              copy.device_addr, copy.size);
                                }
                            }
                        });
                    scheduler.DeferOperation([this, completed_ranges = merged_ranges] {
            ...
            if (!storm_batch_complete) {
                LOG_WARNING(Render_Vulkan,
                            "LBP3 A23 single-allocation storm batch could not reserve {:#x} "
                            "bytes; using the bounded fallback",
                            total_size_bytes);
            }
        }
        ...
        if (!merged_ranges.empty()) {
            for (const PlannedRange& range : merged_ranges) {
                memory_tracker->RecordGpuReadback(range.begin, range.end - range.begin,
                                                  range.buffer_addr,
                                                  slot_buffers[range.buffer_id].SizeBytes());
                if (range.global_drain) {
                    // Rasterizer::InvalidateMemory handles only the exact triggering fault after
                    // this callback returns. Invalidate every additional alias made CPU-owned by
                    // the global drain before any faulting guest thread is released.
                    texture_cache.InvalidateMemory(range.begin, range.end - range.begin);
                }
            }
        }

        for (FaultReadbackRequest* request : requests) {
            request->finished.release();
        }
        // Requests arriving while Finish blocked are drained as the next wave. The
        // scheduled flag remains set until the queue is observed empty under the lock.
    }
}

template <bool async>
void BufferCache::DownloadBufferMemory(Buffer& buffer, VAddr device_addr, u64 size, bool is_write,
                                       bool allow_preemptive) {
    if (buffer.IsHostImported()) {
        scheduler.EndRendering();
        const auto cmdbuf = scheduler.CommandBuffer();
    ...
    if (allow_preemptive && use_lbp3_preemptive_readbacks &&
        TryConsumePreemptiveDownloads(buffer, device_addr, size, copies, is_write)) {
        return;
    }
    CancelPreemptiveDownloads(device_addr, size);
    ...
    scheduler.DeferOperation([this, device_addr, size, is_write] {
    ...
    if constexpr (!async) {
        ...
        if (use_lbp3_preemptive_readbacks) {
            memory_tracker->RecordGpuReadback(device_addr, size, buffer.CpuAddr(),
                                              buffer.SizeBytes());
        }
    }
}
...
void BufferCache::CopyBuffer(VAddr dst, VAddr src, u32 num_bytes, bool dst_gds, bool src_gds) {
    ...
    auto& dst_buffer = [&] -> const Buffer& {
        ...
        auto& buffer = slot_buffers[buffer_id];
        SynchronizeBuffer(buffer, dst, num_bytes, !use_lbp3_preemptive_readbacks, true);
        texture_cache.InvalidateTexelBufferSync(dst, num_bytes);
        TrackGpuWrite(dst, num_bytes);
        return buffer;
    }();
...
std::pair<Buffer*, u32> BufferCache::ObtainBuffer(VAddr device_addr, u32 size, bool is_written,
    ...
    Buffer& buffer = slot_buffers[buffer_id];
    SynchronizeBuffer(buffer, device_addr, size, is_written && !use_lbp3_preemptive_readbacks,
                      is_texel_buffer);
    if (is_written) {
        if (invalidate_texel_sync) {
            texture_cache.InvalidateTexelBufferSync(device_addr, size);
        }
        TrackGpuWrite(device_addr, size);
    }
...
bool BufferCache::IsRegionGpuModified(VAddr addr, size_t size) {
    return memory_tracker->IsRegionGpuModified(addr, size) ||
           (use_lbp3_preemptive_readbacks && gpu_modified_ranges_pending.Intersects(addr, size));
}
...
void BufferCache::TrackGpuWrite(VAddr device_addr, u64 size) {
    if (size == 0) {
        return;
    }
    ...
    // Do not read-protect GPU destinations while the guest is still constructing
    // this command list. The CPU-visible fence commits these ranges in one pass.
    // Any staged bytes from the preceding GPU generation are now stale.
    CancelPreemptiveDownloads(device_addr, size);
    gpu_modified_ranges_pending.Add(device_addr, size);
}

void BufferCache::CancelPreemptiveDownloads(VAddr device_addr, u64 size) {
    if (!use_lbp3_preemptive_readbacks || size == 0) {
        return;
    }

    preemptive_candidates.Subtract(device_addr, size);
    const VAddr page_begin = PageManager::GetPageAddr(device_addr);
    const VAddr page_end = PageManager::GetNextPageAddr(device_addr + size - 1);
    std::scoped_lock lock{preemptive_downloads_mutex};
    for (VAddr page_addr = page_begin; page_addr < page_end; page_addr += TRACKER_BYTES_PER_PAGE) {
        const auto it = preemptive_downloads.find(page_addr);
        if (it == preemptive_downloads.end()) {
            continue;
        }
        for (const auto& download : it->second) {
            download->cancelled = true;
        }
        preemptive_downloads.erase(it);
    }
}

bool BufferCache::TryConsumePreemptiveDownloads(const Buffer& buffer, VAddr device_addr, u64 size,
                                                std::span<const vk::BufferCopy> copies,
                                                bool is_write) {
    if (!use_lbp3_preemptive_readbacks || copies.empty()) {
        return false;
    }

    RangeSet coverage;
    std::vector<std::shared_ptr<PreemptiveDownload>> downloads;
    {
        std::scoped_lock lock{preemptive_downloads_mutex};
        for (const auto& copy : copies) {
            const VAddr copy_addr = buffer.CpuAddr() + copy.srcOffset;
            const VAddr page_begin = PageManager::GetPageAddr(copy_addr);
            const VAddr page_end = PageManager::GetNextPageAddr(copy_addr + copy.size - 1);
            for (VAddr page_addr = page_begin; page_addr < page_end;
                ...
                for (const auto& download : it->second) {
                    ...
                    coverage.Add(download->device_addr, download->size);
                    if (std::ranges::find(downloads, download) == downloads.end()) {
    ...
    for (const auto& copy : copies) {
        const VAddr copy_addr = buffer.CpuAddr() + copy.srcOffset;
        if (!coverage.Contains(copy_addr, copy.size)) {
            return false;
        }
    }
    ...
    for (const auto& download : downloads) {
        done_tick = std::max(done_tick, download->done_tick);
    }
    scheduler.Wait(done_tick);
    scheduler.PopPendingOperations();

    {
        std::scoped_lock lock{preemptive_downloads_mutex};
        if (std::ranges::any_of(downloads, [](const auto& download) {
                return download->cancelled || !download->ready;
            })) {
            return false;
        }

        for (auto it = preemptive_downloads.begin(); it != preemptive_downloads.end();) {
            auto& page_downloads = it->second;
            std::erase_if(page_downloads, [&](const auto& download) {
                if (std::ranges::find(downloads, download) == downloads.end()) {
                    return false;
                }
                download->cancelled = true;
            ...
            if (page_downloads.empty()) {
                it = preemptive_downloads.erase(it);
            } else {
                ++it;
            }
        }
...
bool BufferCache::PreparePreemptiveDownloads() {
    if (!use_lbp3_preemptive_readbacks) {
        return false;
    }

    std::vector<std::pair<VAddr, VAddr>> candidates;
    preemptive_candidates.ForEach(
        [&](VAddr begin, VAddr end) { candidates.emplace_back(begin, end); });
    preemptive_candidates.Clear();
    if (candidates.empty()) {
        return false;
    }
    struct PlannedCopy {
    ...
    std::vector<PlannedCopy> plans;
    u64 total_size_bytes = 0;
    bool batch_full = false;

    for (const auto [candidate_begin, candidate_end] : candidates) {
        gpu_modified_ranges.ForEachInRange(
            candidate_begin, candidate_end - candidate_begin,
            [&](VAddr dirty_begin, VAddr dirty_end) {
                const auto plan_page = [&](VAddr page_addr) {
                    if (batch_full) {
                        return;
                    }
                    const VAddr start_addr = std::max(page_addr, dirty_begin);
                    const VAddr end_addr =
                        std::min<VAddr>(page_addr + TRACKER_BYTES_PER_PAGE, dirty_end);
                    if (start_addr >= end_addr) {
                        return;
                    }
                    const BufferId buffer_id = page_table[start_addr >> CACHING_PAGEBITS].buffer_id;
                    if (IsBufferInvalid(buffer_id)) {
                        return;
                    }
                    const Buffer& buffer = slot_buffers[buffer_id];
                    const u64 copy_size = end_addr - start_addr;
                    if (!buffer.IsInBounds(start_addr, copy_size)) {
                        return;
                    }
                    const u64 staging_offset = Common::AlignUp(total_size_bytes, 64ULL);
                    if (staging_offset + copy_size > DownloadBufferSize) {
                        batch_full = true;
                        return;
                    }
                    plans.push_back({buffer_id, start_addr, copy_size, staging_offset});
                    total_size_bytes = staging_offset + copy_size;
                };

                memory_tracker->ForEachPreemptiveReadbackPage(dirty_begin, dirty_end - dirty_begin,
                                                              plan_page);
            });
    }
    ...
    if (batch_full) {
        LOG_WARNING(Render_Vulkan,
                    "LBP3 preemptive readback batch reached the {:#x}-byte ring "
                    "limit",
                    DownloadBufferSize);
    }

    const auto [staging, download_offset] = download_buffer.Map(total_size_bytes, 64);
    if (staging == nullptr) {
        LOG_WARNING(Render_Vulkan,
                    "Could not reserve {:#x} bytes for an LBP3 preemptive "
                    "readback batch",
                    total_size_bytes);
        return false;
    }
    ...
    std::vector<std::shared_ptr<PreemptiveDownload>> downloads;
    downloads.reserve(plans.size());
    std::unordered_map<BufferId, std::vector<vk::BufferCopy>> copies_by_buffer;
    {
        std::scoped_lock lock{preemptive_downloads_mutex};
        for (const PlannedCopy& plan : plans) {
            auto download = std::make_shared<PreemptiveDownload>(PreemptiveDownload{
                .device_addr = plan.device_addr,
                .size = plan.size,
                .staging = staging + plan.staging_offset,
                .done_tick = done_tick,
            });
            downloads.push_back(download);
            const VAddr page_addr = PageManager::GetPageAddr(plan.device_addr);
            preemptive_downloads[page_addr].push_back(download);

            const Buffer& buffer = slot_buffers[plan.buffer_id];
            auto& buffer_copies = copies_by_buffer[plan.buffer_id];
            const vk::BufferCopy copy{
                .srcOffset = buffer.Offset(plan.device_addr),
                .dstOffset = download_offset + plan.staging_offset,
                .size = plan.size,
            };
            if (!buffer_copies.empty() &&
                buffer_copies.back().srcOffset + buffer_copies.back().size == copy.srcOffset &&
                buffer_copies.back().dstOffset + buffer_copies.back().size == copy.dstOffset) {
                buffer_copies.back().size += copy.size;
            } else {
    ...
    const auto cmdbuf = scheduler.CommandBuffer();
    boost::container::small_vector<vk::BufferMemoryBarrier2, 16> pre_barriers;
    for (auto& [buffer_id, copies] : copies_by_buffer) {
        Buffer& buffer = slot_buffers[buffer_id];
    ...
    for (const auto& [buffer_id, copies] : copies_by_buffer) {
        cmdbuf.copyBuffer(slot_buffers[buffer_id].Handle(), download_buffer.Handle(), copies);
    }
    const vk::BufferMemoryBarrier2 host_barrier = {
        .srcStageMask = vk::PipelineStageFlagBits2::eTransfer,
        .srcAccessMask = vk::AccessFlagBits2::eTransferWrite,
    ...
    scheduler.DeferOperation([this, downloads = std::move(downloads)] {
        std::scoped_lock lock{preemptive_downloads_mutex};
        for (const auto& download : downloads) {
            if (download->cancelled) {
                continue;
            }
            if (!memory->TryWriteBacking(std::bit_cast<u8*>(download->device_addr),
...
void BufferCache::CommitPendingGpuRanges() {
    ...
    std::vector<std::pair<VAddr, VAddr>> pending_ranges;
    gpu_modified_ranges_pending.ForEach(
        [&](VAddr begin, VAddr end) { pending_ranges.emplace_back(begin, end); });
    if (pending_ranges.empty()) {
        return;
    }

    // SynchronizeBuffer deliberately excludes pending GPU destinations from CPU
    // uploads. It still clears their CPU-dirty state and marks/protects them as
    // GPU modified, but only now that the command stream exposed a real fence.
    for (const auto [begin, end] : pending_ranges) {
        SynchronizeBuffersInRange(begin, end - begin, true);
        gpu_modified_ranges.Add(begin, end - begin);
        preemptive_candidates.Add(begin, end - begin);
    }
...
bool BufferCache::SynchronizeBuffer(Buffer& buffer, VAddr device_addr, u32 size, bool is_written,
    ...
    memory_tracker->ForEachUploadRange(
        ...
        [&](u64 device_addr_out, u64 range_size) {
            ...
            if (use_lbp3_preemptive_readbacks) {
                // A pending range is the destination of already-recorded GPU work.
                // Uploading later CPU writes over it at fence time would overwrite
                // the result before the GPU has produced it.
                gpu_modified_ranges_pending.ForEachNotInRange(device_addr_out, range_size,
                                                              add_upload);
            } else {
...
void BufferCache::DeleteBuffer(BufferId buffer_id) {
    ...
    CancelPreemptiveDownloads(buffer.CpuAddr(), buffer.SizeBytes());
